import { CardProps } from "@/components/team/execom";

const eventsData: CardProps[] = [
  {
    username: "Maliha Ishaq",
    dp: "/assets/execom/maliha.webp",
    position: "Events",
    instalink: "https://www.instagram.com/ishaaaa__05",
    linkedlinlink: "https://www.linkedin.com/in/syeda-maliha-ishaq-7b4273262/",
    twitterlink: "",
    head: true,
    insta: true,
    linkedln: true,
    twitter: false,
  },
  {
    username: "Mohammad Yaseen",
    dp: "/assets/execom/yaseen.jpeg",
    position: "Events",
    instalink: "https://www.instagram.com/i.yaseen.07",
    linkedlinlink: "https://www.linkedin.com/in/yaseen-mohammed-5a4ba925b/",
    twitterlink: "",
    head: true,
    insta: true,
    linkedln: true,
    twitter: false,
  },
  {
    username: "Mohammed Umar Salam",
    dp: "/assets/core/umar.webp",
    position: "Events",
    instalink: "https://www.instagram.com/_mdumar_18",
    linkedlinlink: "",
    twitterlink: "",
    insta: true,
    head: false,
    linkedln: false,
    twitter: false,
  },
  {
    username: "Syed Alyan Ahmed",
    dp: "/assets/core/alyan.webp",
    position: "Events",
    instalink: "https://www.instagram.com/s.a_alyan",
    linkedlinlink: "",
    twitterlink: "",
    insta: true,
    head: false,
    linkedln: false,
    twitter: false,
  },
  {
    username: "Muzammil Shareef",
    dp: "/assets/core/muzammil.webp",
    position: "Events",
    instalink: "https://www.instagram.com/Koi.muzammil.gaya",
    linkedlinlink: "",
    twitterlink: "",
    insta: true,
    head: false,
    linkedln: false,
    twitter: false,
  },
];

export default eventsData;
